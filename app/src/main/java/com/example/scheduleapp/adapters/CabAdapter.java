...
public class CabAdapter extends RecyclerView.Adapter<CabAdapter.SearchViewHolder> {

    private Context context;
    private List<Cab> list;

    public CabAdapter(Context context, List<Cab> list) {
    ...
    @NonNull
    @Override
    public CabAdapter.SearchViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.search_item, parent, false);
        return new SearchViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull CabAdapter.SearchViewHolder holder, int position) {
        ...
        holder.itemSearchCardView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    list2 = mainRepository.getScheduleByCab(list.get(position).getCabNum(), sdf.format(date));
        ...
        holder.itemSearchCardView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                if(holder.searchStarImageView.getTag().equals("1")) {
                    SplashScreenActivity.removeFavoriteCab(list.get(position));
                    list.get(position).setFavorite(false);
                    holder.searchStarImageView.setTag("0");
                    holder.searchStarImageView.setVisibility(View.INVISIBLE);
                } else if(holder.searchStarImageView.getTag().equals("0")){
                    SplashScreenActivity.saveCab(list.get(position));
